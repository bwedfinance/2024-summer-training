...
ms_model3 <- ms_data |>
  ...
  mutate(sparsity_weight_adj = case_when(st_per_sq_mile > sparsity_limit ~ 0,
         ...
         # total Model 3 funding 
         model_3_total_funding = base_funding + total_sped_funding + ell_funding + 
           ed_funding + gifted_funding + vocational_funding + conc_pov_funding + 
           gr_9_12_funding + sparsity_funding,
         
         model_3_pp_funding = model_3_total_funding / total_enroll, 
         
         # Model 3 local share with the mills 
         model_3_local_mills_total = mill_1_value * max_mills,
         
         # Model 3 local share with 28% 
         model_3_local_pct_total = model_3_total_funding * max_local_pct, 
         
         # Apply the if_else to calculate the local share 
         model_3_local_share_total = ifelse(model_3_local_mills_total < model_3_local_pct_total, 
                                            model_3_local_mills_total, model_3_local_pct_total),
         model_3_local_share_pp = model_3_local_share_total / total_enroll,
         
         # Model 3 State share
         model_3_state_share = model_3_total_funding  - model_3_local_share_total,
         model_3_state_share_pp = model_3_state_share / total_enroll) |> 
  
  # Model 3 and current funding differences 
  mutate(model_3_diff_total = model_3_total_funding - maep_expected_local_total, 
         model_3_diff_pp = model_3_pp_funding - maep_expected_local_pp, 
         
         model_3_local_diff_total = model_3_local_share_total - local_revenue, 
         current_local_pp = local_revenue / total_enroll, 
         model_3_local_diff_pp = model_3_local_share_pp - current_local_pp, 
         
         model_3_state_diff_total = model_3_state_share - maep_rev_full_funding_fy24, 
         model_3_state_diff_pp = model_3_state_share_pp - maep_full_pp_fy24) |>
  ...
  mutate(sped_pp_funding = sped_funding / (sped_tier_1_enroll + sped_tier_2_enroll + sped_tier_3_enroll),
         sped_model_3_pp = total_sped_funding / (sped_tier_1_enroll + sped_tier_2_enroll + sped_tier_3_enroll),
         sped_pp_diff = sped_model_3_pp - sped_pp_funding) |>
  # Difference in per-pupil at risk funding 
  mutate(at_risk_pp_funding = at_risk_funding / econ_dis_enroll,
         ed_model_3_pp = ed_funding / econ_dis_enroll,
         ed_pp_diff = ed_model_3_pp - at_risk_pp_funding) |>
  # Difference in per-pupil vocational funding  
  mutate(cte_pp_funding = cte_funding / vocational_enroll,
         cte_model_3_pp = vocational_funding / vocational_enroll,
         cte_pp_diff = cte_model_3_pp - cte_pp_funding) |>
  ...
  select(dist_id, district, model_3_diff_total, model_3_diff_pp, ell_funding, 
         sparsity_funding, conc_pov_funding, model_3_total_funding, model_3_local_share_total, 
         model_3_state_share_pp, everything())
...
ggplot() +
  geom_point(data = ms_model3,
             aes(x = maep_expected_local_pp, y = model_3_pp_funding,
                 size = total_enroll, color = direct_cert_pct),
             alpha = .8) +
  geom_text_repel(data = ms_model3 |>
                    filter(model_3_pp_funding > 9175),
                  aes(x = maep_expected_local_pp, y =  model_3_pp_funding,
                      label = district),
                  size = text_repel_size) +
  geom_text_repel(data = ms_model3 |>
                    filter(model_3_pp_funding < 7750),
                  aes(x = maep_expected_local_pp, y =  model_3_pp_funding,
                      label = district),
                  size = text_repel_size) +
  geom_text_repel(data = ms_model3 |>
                    filter(maep_expected_local_pp > 8100 &
                             model_3_pp_funding < 9150),
                  aes(x = maep_expected_local_pp, y = model_3_pp_funding,
                      label = district),
                  size = text_repel_size) +
  ...
  labs(x = "Total (MAEP + Expected Local) Funding Per-Pupil, FY24",
       y = "Model 3 Funding Per-Pupil, FY24",
       color = "Direct Cert %",
       size = "Enrollment",
       # shape = "LEA Type",
       title = "Change in FY24 Per-Pupil Funding, Model 3") +
  theme_bw() +
  theme(text = element_text(family = "Avenir", size = 11),
        plot.caption = element_text(hjust = 0))

ggsave("figures/ms_model3/model3_vs_maep_local_expected.png", units = "in",
       height = 5.9, width = 10)

# SCATTERPLOT #2: Model 3 AND DIRECT CERT % -------
ggplot(ms_model3, aes(x = direct_cert_pct, y = model_3_pp_funding, size = total_enroll, color = urbanicity)) +
  geom_point(alpha = .8) +
  ...
  labs(x = "Direct Certification Percent, FY23",
       y = "Model 3 Per-Pupil Funding, FY24",
...
ggsave("figures/ms_model3/model3_pp_direct_cert_pct.png", units = "in",
       height = 5.9, width = 10)

# SCATTERPLOT #3: MODEL 3 + VALUE OF 1 MILL ----
ggplot(ms_model3, aes(x = mill_1_value_pp, y = model_3_pp_funding, size = total_enroll, color = direct_cert_pct)) +
  geom_point(alpha = .8) +
  scale_color_viridis(labels = percent_format()) +
...
ggsave("figures/ms_model3/model3_model_1_pp_value_1_mill.png", units = "in",
       height = 5.9, width = 10)

# Mapping --------

# merge model with map data
ms_model3_dist_map <- ms_shp |>
  left_join(ms_model3, by ="geoid") |>
  ...
  select(dist_id, model_3_diff_pp, everything())


# Per Pupil funding diff map
ggplot(ms_model3_dist_map) +
  geom_sf(aes(fill = model_3_diff_pp),
          color = "grey54",
          size = .5) +
...
ggsave("figures/ms_model3/pp_diff_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/ms_model3/ell_pp_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/ms_model3/sparsity_pp_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/ms_model3/conc_pov_pp_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/ms_model3/sped_pp__diff_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/ms_model3/ed_pp_diff_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/ms_model3/cte_pp_diff_map.png", units = "in",
       height = 5.9, width = 6)


# state summary --------
model3_state_summary <- ms_model3 |>
  summarise(model_3_total_funding = sum(model_3_total_funding, na.rm = T),
            ...
            model_3_state_share = sum(model_3_state_share, na.rm = T)) 

# model 2 data for mississippi working session -----
ms_model3_working_session <- ms_model3 |>
  select(-geoid, -model_3_local_diff_pp, -model_3_state_diff_total, - shortName,
         ...
         -local_revenue_total, -local_revenue_pp, - mpv, -mhi, -model_3_local_pct_total, 
         -current_local_pp, -ed_model_3_pp, -local_revenue, -federal_revenue,
         -local_share_pp_pct, -urbanicity, -at_risk_pp_funding, 
         -sped_pp_funding, -sped_model_3_pp, - model_3_state_diff_pp) |>
  select(dist_id, district, total_enroll, model_3_total_funding, model_3_pp_funding, 
         model_3_diff_total, model_3_diff_pp, model_3_state_share,  model_3_state_share_pp, 
         model_3_local_diff_total, model_3_local_share_total, model_3_local_share_pp, 
         model_3_local_diff_total, base_funding, sped_tier_1_funding, sped_tier_2_funding, 
...
write_csv(ms_model3, "data/processed/ms_model3_data.csv")

write_csv(ms_model3_working_session, "data/processed/ms_model3_2023-12-14_data.csv")
