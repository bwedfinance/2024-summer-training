...
ms_model5 <- ms_data |>
  ...
  mutate(sparsity_weight_adj = case_when(st_per_sq_mile > sparsity_limit ~ 0,
         ...
         # total Model 5 funding 
         model_5_total_funding = base_funding + total_sped_funding + ell_funding + 
           ed_funding + gifted_funding + vocational_funding + sparsity_funding,
         
         model_5_pp_funding = model_5_total_funding / total_enroll, 
         
         # Model 5 local share with the mills 
         model_5_local_mills_total = mill_1_value * max_mills,
         
         # Model 5 local share with 28% 
         model_5_local_pct_total = model_5_total_funding * max_local_pct, 
         
         # Apply the if_else to calculate the local share 
         model_5_local_share_total = ifelse(model_5_local_mills_total < model_5_local_pct_total, 
                                            model_5_local_mills_total, model_5_local_pct_total),
         model_5_local_share_pp = model_5_local_share_total / total_enroll,
         
         # Model 5 State share
         model_5_state_share = model_5_total_funding  - model_5_local_share_total,
         model_5_state_share_pp = model_5_state_share / total_enroll) |> 
  
  # Model 5 and current funding differences 
  mutate(model_5_diff_total = model_5_total_funding - maep_expected_local_total, 
         model_5_diff_pp = model_5_pp_funding - maep_expected_local_pp, 
         
         model_5_local_diff_total = model_5_local_share_total - local_revenue, 
         current_local_pp = local_revenue / total_enroll, 
         model_5_local_diff_pp = model_5_local_share_pp - current_local_pp, 
         
         model_5_state_diff_total = model_5_state_share - maep_rev_full_funding_fy24, 
         model_5_state_diff_pp = model_5_state_share_pp - maep_full_pp_fy24) |>
  ...
  mutate(sped_pp_funding = sped_funding / (sped_tier_1_enroll + sped_tier_2_enroll + sped_tier_3_enroll),
         sped_model_5_pp = total_sped_funding / (sped_tier_1_enroll + sped_tier_2_enroll + sped_tier_3_enroll),
         sped_pp_diff = sped_model_5_pp - sped_pp_funding) |>
  # Difference in per-pupil at risk funding 
  mutate(at_risk_pp_funding = at_risk_funding / econ_dis_enroll,
         ed_model_5_pp = ed_funding / econ_dis_enroll,
         ed_pp_diff = ed_model_5_pp - at_risk_pp_funding) |>
  # Difference in per-pupil vocational funding  
  mutate(cte_pp_funding = cte_funding / vocational_enroll,
         cte_model_5_pp = vocational_funding / vocational_enroll,
         cte_pp_diff = cte_model_5_pp - cte_pp_funding) |>
  ...
  select(dist_id, district, model_5_diff_total, model_5_diff_pp, ell_funding, 
         sparsity_funding, model_5_total_funding, model_5_local_share_total, 
         model_5_state_share_pp, everything())
...
#
# ggplot() +
#   geom_point(data = ms_model5,
#              aes(x = maep_expected_local_pp, y = model_5_pp_funding,
#                  size = total_enroll, color = direct_cert_pct),
#              alpha = .8) +
#   geom_text_repel(data = ms_model5 |>
#                     filter(model_5_pp_funding > 8000),
#                   aes(x = maep_expected_local_pp, y =  model_5_pp_funding,
#                       label = district),
...
#                   size = text_repel_size) +
#   geom_text_repel(data = ms_model5 |>
#                     filter(model_5_pp_funding < 7100),
#                   aes(x = maep_expected_local_pp, y =  model_5_pp_funding,
#                       label = district),
#                   size = text_repel_size) +
#   geom_text_repel(data = ms_model5 |>
#                     filter(maep_expected_local_pp > 7400 &
#                              model_5_pp_funding < 7550),
#                   aes(x = maep_expected_local_pp, y = model_5_pp_funding,
#                       label = district),
#                   size = text_repel_size) +
...
#        y = "Model 5 Funding Per-Pupil, FY24",
#        color = "Direct Cert %",
#        size = "Enrollment",
#        # shape = "LEA Type",
#        title = "Change in FY24 Per-Pupil Funding, Model 5") +
#   theme_bw() +
#   theme(text = element_text(family = "Avenir", size = 11),
#         plot.caption = element_text(hjust = 0))
#
# ggsave("figures/ms_model5/model5_vs_maep_local_expected.png", units = "in",
#        height = 5.9, width = 10)
#
# # SCATTERPLOT #2: Model 5 AND DIRECT CERT % -------
# ggplot(ms_model5, aes(x = direct_cert_pct, y = model_5_pp_funding, size = total_enroll, color = urbanicity)) +
#   geom_point(alpha = .8) +
...
#        y = "Model 5 Per-Pupil Funding, FY24",
...
#
# ggsave("figures/ms_model5/model5_pp_direct_cert_pct.png", units = "in",
#        height = 5.9, width = 10)
#
# # SCATTERPLOT #3: Model 5 + VALUE OF 1 MILL ----
# ggplot(ms_model5, aes(x = mill_1_value_pp, y = model_5_pp_funding, size = total_enroll, color = direct_cert_pct)) +
#   geom_point(alpha = .8) +
#   scale_color_viridis(labels = percent_format()) +
...
#         plot.caption = element_text(hjust = 0, color = bw_primary[5], size = 10))
#
# ggsave("figures/ms_model5/model5_model_1_pp_value_1_mill.png", units = "in",
#        height = 5.9, width = 10)
#
# # Mapping --------
#
# # merge model with map data
# ms_model5_dist_map <- ms_shp |>
#   left_join(ms_model5, by ="geoid") |>
...
#   select(dist_id, model_5_diff_pp, everything())
#
#
# # Per Pupil funding diff map
# ggplot(ms_model5_dist_map) +
#   geom_sf(aes(fill = model_5_diff_pp),
#           color = "grey54",
#           size = .5) +
...
#         plot.caption = element_text(hjust = 0))
#
# ggsave("figures/ms_model5/pp_diff_map.png", units = "in",
#        height = 5.9, width = 6)
#
...
#         plot.caption = element_text(hjust = 0))
#
# ggsave("figures/ms_model5/ell_pp_map.png", units = "in",
#        height = 5.9, width = 6)
#
#
...
#         plot.caption = element_text(hjust = 0))
#
# ggsave("figures/ms_model5/sparsity_pp_map.png", units = "in",
#        height = 5.9, width = 6)
#
...
# #         plot.caption = element_text(hjust = 0))
# #
# # ggsave("figures/ms_model5/conc_pov_pp_map.png", units = "in",
# #        height = 5.9, width = 6)
#
...
#         plot.caption = element_text(hjust = 0))
#
# ggsave("figures/ms_model5/sped_pp__diff_map.png", units = "in",
#        height = 5.9, width = 6)
#
#
...
#         plot.caption = element_text(hjust = 0))
#
# ggsave("figures/ms_model5/ed_pp_diff_map.png", units = "in",
#        height = 5.9, width = 6)
#
...
#         plot.caption = element_text(hjust = 0))
#
# ggsave("figures/ms_model5/cte_pp_diff_map.png", units = "in",
#        height = 5.9, width = 6)
#
#
# # state summary --------
# model5_state_summary <- ms_model5 |>
#   summarise(model_5_total_funding = sum(model_5_total_funding, na.rm = T),
...
#             model_5_state_share = sum(model_5_state_share, na.rm = T))

# model 2 data for mississippi working session -----
ms_model5_working_session <- ms_model5 |>
  select(-geoid, -model_5_local_diff_pp, -model_5_state_diff_total, - shortName,
         ...
         -local_revenue_total, -local_revenue_pp, - mpv, -mhi, -model_5_local_pct_total,
         -current_local_pp, -ed_model_5_pp, -local_revenue, -federal_revenue,
         -local_share_pp_pct, -urbanicity, -at_risk_pp_funding,
         -sped_pp_funding, -sped_model_5_pp, - model_5_state_diff_pp) |>
  select(dist_id, district, total_enroll, model_5_total_funding, model_5_pp_funding,
         model_5_diff_total, model_5_diff_pp, model_5_state_share,  model_5_state_share_pp,
         model_5_local_diff_total, model_5_local_share_total, model_5_local_share_pp,
         model_5_local_diff_total, base_funding, sped_tier_1_funding, sped_tier_2_funding,
...
write_csv(ms_model5, "data/processed/ms_model5_data.csv")

write_csv(ms_model5_working_session, "data/processed/ms_model5_2023-12-14_data.csv")
