# ms_model9.R
# last updated by Krista Kaput on 2024-02-20
...
ms_data <- read_csv("data/processed/current_modeling_addon_updated_charter.csv") |>
  select(dist_id, district, econ_dis_enroll, stpov_enroll, everything())


ms_data$econ_dis_enroll <- round(ms_data$econ_dis_enroll, digits = 0) 

ms_data$stpov_enroll <- round(ms_data$stpov_enroll, digits = 0) 
...
ms_model9 <- ms_data |>
  ...
  mutate(model_9_diff_total = model_9_total_funding - maep_expected_local_total, 
         ...
         model_9_state_total_diff = model_9_state_share - maep_rev_fy24, 
         model_9_local_diff_pp = model_9_local_share_pp - current_local_pp,
         model_9_state_diff_pp = model_9_state_share_pp - maep_fy24_pp) |>
  ...
  mutate(sped_pp_funding = sped_funding / (sped_tier_1_enroll + sped_tier_2_enroll + sped_tier_3_enroll),
         sped_model_9_pp = total_sped_funding / (sped_tier_1_enroll + sped_tier_2_enroll + sped_tier_3_enroll),
         sped_pp_diff = sped_model_9_pp - sped_pp_funding) |>
  # Difference in per-pupil at risk funding 
  mutate(at_risk_pp_funding = at_risk_funding / econ_dis_enroll,
         ed_model_9_pp = ed_funding / econ_dis_enroll,
         ed_pp_diff = ed_model_9_pp - at_risk_pp_funding) |>
  # Difference in per-pupil vocational funding  
  mutate(cte_pp_funding = cte_funding / vocational_enroll,
         cte_model_9_pp = vocational_funding / vocational_enroll,
         cte_pp_diff = cte_model_9_pp - cte_pp_funding) |>
  
  
  # Take out Forest and charter schools 
  filter(mill_1_value_pp < 1000) |>
  
  select(dist_id, district, model_9_state_diff_pp, maep_rev_fy24, model_9_state_total_diff, 
         model_9_state_diff_pp, model_9_state_share_pp, maep_fy24_pp, 
         model_9_diff_pp, model_9_state_diff_pp, model_9_pp_funding, model_9_local_share_total, 
         model_9_diff_total, base_funding, sped_tier_1_funding, sped_tier_2_funding, sped_tier_3_funding, 
...
ms_model9_charters <- ms_model9 |>
  filter(dist_id == "2545" | dist_id == "1425" | dist_id == "4225" | 
           dist_id == "2525" | dist_id == "2515" | dist_id == "2535" | dist_id == "2505") |> 
  select(dist_id, district,  model_9_state_diff_pp, maep_rev_fy24, model_9_state_total_diff, 
         model_9_state_diff_pp, model_9_state_share_pp, model_9_pp_funding, maep_expected_local_pp, 
         model_9_diff_pp, vocational_enroll)


# Winners and losers -------
# 
# # create gain loss summary
# wsf_model9_gain_loss <- ms_model9 |>
#   mutate(gain_loss = ifelse(model_9_total_funding > current_funding_total,
#                             "Gain", "Lose")) |>
#   group_by(gain_loss) |>
#   summarise(n_leas = n(),
#             enroll = sum(total_adm, na.rm = T),
#             frpl = sum(free_reduced, na.rm = T)) |>
#   mutate(frpl_pct = frpl / enroll)
...
ggplot() +
  ...
  labs(x = "Total (MAEP + Expected Local) Funding Per-Pupil, FY24",
       y = "Model Funding Per-Pupil, FY24",
       color = "Direct Cert %",
       size = "Enrollment",
       # shape = "LEA Type",
       title = "FIGURE 4: Change in FY24 Per-Pupil Funding") +
  theme_bw() +
  theme(text = element_text(family = "Avenir", size = 11),
        plot.caption = element_text(hjust = 0))

ggsave("figures/updated_ms_model9/model9_vs_maep_local_expected.png", units = "in",
       height = 5.9, width = 10)

# SCATTERPLOT #2: Model 9 AND DIRECT CERT % -------
ggplot(ms_model9, aes(x = direct_cert_pct, y = model_9_pp_funding, size = total_enroll, color = urbanicity)) +
  geom_point(alpha = .8) +
  ...
  labs(x = "Direct Certification Percent, FY23",
       y = "Model Per-Pupil Funding, FY24",
       title = "FIGURE 6: Proposed Model Per-Pupil Funding, FY24",
       size = "Enrollment",
       color = "Urbanicity") +
  theme_bw() +
...
ggsave("figures/updated_ms_model9/model9_pp_direct_cert_pct.png", units = "in",
       height = 5.9, width = 10)

# Updated MAEP PP ------
ggplot(ms_model9, aes(x = direct_cert_pct, y = maep_expected_local_pp, size = total_enroll, color = urbanicity)) +
  geom_point(alpha = .8) +
  scale_color_manual(values = c("#6D1E4A", "#007786",
                                "#FFC762", "#212B46")) +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
  scale_y_continuous(labels = scales::dollar_format(accuracy = 1),
                     limits = c(5000, 11000)) +
  # scale_size_area(labels = comma, max_size = 10) +
  labs(x = "Direct Certification % (Proxy measure for student poverty), FY23",
       y = "Total Current (MAEP + Expected Local) Funding Per-Pupil, FY24",
       title = "FIGURE 5: MS Total Current (MAEP + Expected Local) Per-Pupil Funding, FY24",
       size = "Enrollment",
       color = "Urbanicity") +
  theme_bw() +
  theme(text = element_text(family = "Avenir", size = 12),
...
ggsave("figures/updated_ms_model9/model9_maep_pp_direct_cert_pct.png", units = "in",
       height = 5.9, width = 10)

# SCATTERPLOT #3: MODEL 3 + VALUE OF 1 MILL ----
ggplot(ms_model9, aes(x = mill_1_value_pp, y = model_9_pp_funding, size = total_enroll, color = direct_cert_pct)) +
  geom_point(alpha = .8) +
  scale_color_viridis(labels = percent_format()) +
  ...
  labs(x = "Value of 1 Mill Per-Pupil, FY23",
       y = "Model 9 Per-Pupil Funding, FY24",
...
ggsave("figures/updated_ms_model9/model9_model_1_pp_value_1_mill.png", units = "in",
       height = 5.9, width = 10)

# Mapping --------

# merge model with map data
ms_model9_dist_map <- ms_shp |>
  left_join(ms_model9, by ="geoid") |>
  ...
  select(dist_id, model_9_diff_pp, everything())


# Per Pupil funding diff map
ggplot(ms_model9_dist_map) +
  geom_sf(aes(fill = model_9_diff_pp),
          color = "grey54",
          size = .5) +
...
ggsave("figures/updated_ms_model9/pp_diff_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/updated_ms_model9/ell_pp_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/updated_ms_model9/sparsity_pp_map.png", units = "in",
       height = 5.9, width = 6)
...
#         plot.caption = element_text(hjust = 0))
# 
# ggsave("figures/ms_model9/conc_pov_pp_map.png", units = "in",
#        height = 5.9, width = 6)
...
ggsave("figures/updated_ms_model9/sped_pp__diff_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/updated_ms_model9/ed_pp_diff_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/updated_ms_model9/cte_pp_diff_map.png", units = "in",
       height = 5.9, width = 6)


# State Share funding diff map --------

bw_state_revenue <- c("#6D1E4A", "#FFA497",  "#007786", "#0D525A") 

ggplot(ms_model9_dist_map) +
  geom_sf(aes(fill = model_9_state_diff_pp),
          color = "grey54",
          size = .5) +
  scale_fill_viridis(na.value = "grey84",
                     labels = dollar_format()) +
  theme_void() +
  scale_fill_stepsn(breaks =c (-1800, -750, 0, 750, 1800),
                    colors = bw_state_revenue,
                    name = "Difference in per-pupil state share",
                    labels = dollar_format(accuracy = 1L)) +

  labs(fill = "Difference in state share per-pupil funding") +
  theme(text = element_text(family = "Avenir", size = 11),
        plot.caption = element_text(hjust = 0))

ggsave("figures/updated_ms_model9/state_share_pp_diff_map.png", units = "in",
       height = 5.9, width = 6)
...
ms_model9_working_session <- ms_model9 |>
  select(-geoid, -model_9_local_diff_pp, - shortName,
         ...
         -local_revenue_total, -local_revenue_pp, - mpv, -mhi, -model_9_local_pct_total, 
         -current_local_pp, -ed_model_9_pp, -local_revenue, -federal_revenue,
         -local_share_pp_pct, -urbanicity, -at_risk_pp_funding, -model_9_local_diff_total,
         -sped_pp_funding, -sped_model_9_pp, - model_9_state_diff_pp, -maep_rev_fy23,
         -maep_fy23_pp, -cte_model_9_pp, -cte_pp_diff) |>
  select(dist_id, district, total_enroll, model_9_total_funding, model_9_pp_funding, 
         model_9_diff_total, model_9_diff_pp, model_9_state_share,  model_9_state_share_pp, 
         model_9_state_total_diff, model_9_local_share_total, model_9_local_share_pp, model_9_local_mills_total,
...
# CSV File for Rachel 2024-02-20 ------

ms_6600_base <- ms_model9 |>
  select(dist_id, district, stpov_rate, stpov_enroll, direct_cert_pct, econ_dis_enroll, 
         at_risk_funding, ed_funding, conc_pov_funding) |>
  rename(direct_cert_enroll = econ_dis_enroll,
         model_econ_dis_funding = ed_funding) 


# export csv ------

# write_csv(ms_model9, "data/processed/ms_model9_matt_updates_addon_data.csv")

write_csv(ms_model9_working_session, "data/processed/ms_6600_base_2024-02-20_data.csv")

write_csv(ms_6600_base, "data/processed/ms_6600_base_data.csv")
