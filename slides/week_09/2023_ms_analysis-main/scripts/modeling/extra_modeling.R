...
ms_shp <- st_read(here::here("data/processed/ms_shp.shp"))
...
ms_model1 <- ms_data |>
  ...
  select(dist_id, district, total_sped_funding, sped_tier_1_enroll, 
         sped_tier_2_enroll, sped_tier_3_enroll, sped_tier_1_funding, 
         sped_tier_2_funding, sped_tier_3_funding, model_1_diff_total, 
         model_1_state_local_total, model_1_state_local_pp, model_1_total_funding, 
...
ggplot() +
  ...
  geom_text_repel(data = ms_model1 |>
                    filter(maep_expected_local_pp < 7500,
                           model_1_pp_funding > 7550),
                  aes(x = maep_expected_local_pp, y = model_1_pp_funding,
                      label = district),
                  size = text_repel_size) +
  geom_text_repel(data = ms_model1 |>
                    filter(maep_expected_local_pp > 7900),
                  aes(x = maep_expected_local_pp, y = model_1_pp_funding,
                      label = district),
                  size = text_repel_size) +
  # geom_text_repel(data = ms_model1 |>
...
#   select(dist_id, model_1_diff_pp, everything())
# 
# 
