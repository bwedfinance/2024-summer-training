...
# updated enrollment 
ms_updated_enroll_raw <- read_csv("data/raw/FinanceSimulationData (2).csv")

# load in the ada addon data
ada_addon_raw <- read_excel("data/raw/fy24_average_daily_enrollment_allocation_notice_alpha_district_0.xlsx", n_max = 148)

# load in the teacher addon data
teacher_pay_addon_raw <- read_excel("data/raw/fy2024_teacher_asst._teacher_pay_allocation_notice_district_alpha (2).xlsx", 
                                    skip = 2, n_max = 145)
...
# MS ADA add on 
ada_addon <- ada_addon_raw |> 
  rename(dist_id = "District Number",
         ada_addon_fy24 = "FY2024 Allocation Amount") |>
  mutate(dist_id = str_pad(dist_id, 4, pad = "0", side = "left")) |>
  select(dist_id, ada_addon_fy24) 

# MS Teacher Allocation add on 
teacher_pay_addon <- teacher_pay_addon_raw |>
  rename(dist_id = "District Number",
         teacher_addon_fy24 = "FY2024 Allocation Amount") |>
  mutate(dist_id = str_pad(dist_id, 4, pad = "0", side = "left")) |>
  select(dist_id, teacher_addon_fy24)
  

# MS updated enrollment data
ms_updated_enroll <- ms_updated_enroll_raw |>
  rename_with(tolower) |>
  rename(dist_id = id, 
         district = name, 
         total_enroll = "total enrollment (fy23)") |>
  mutate(dist_id = str_pad(dist_id, 4, pad = "0", side = "left")) |>
  select(dist_id, total_enroll)
...
ms_district_clean <- ms_district_raw |>
  ...
  left_join(ms_updated_enroll, by = "dist_id") |>
...
# Join and then mutate to calculate the updated FY24 MAEP data 
  
ms_district_clean <- ms_district_clean |>
  left_join(ada_addon, by = "dist_id") |>
  left_join(teacher_pay_addon, by = "dist_id") |>
  mutate(maep_fy23_pp = maep_rev_fy23/total_enroll, 
         maep_rev_fy24 = maep_rev_fy24 + ada_addon_fy24 + teacher_addon_fy24, 
         maep_rev_full_funding_fy24 = maep_rev_full_funding_fy24 + ada_addon_fy24 + teacher_addon_fy24, 
...
current_modeling_data <- ms_district_clean |>
  ...
  mutate(local_share_expected_total = ifelse(local_funding_28_mill < local_funding_27_pct, 
         ...
         local_diff_pp = local_revenue_pp- local_share_expected_pp) |>
  select(geoid, dist_id, district, total_enroll, local_diff_pp, maep_expected_local_total, maep_expected_local_pp, maep_rev_fy24, 
         local_share_expected_total, local_funding_28_mill, local_funding_27_pct, everything()) 

# # Scatterplot of MAEP + Actual Local --------
...
ggplot() +
  ...
  geom_text_repel(data = current_modeling_data |>
                    filter(rev_pp_local_state > 11800),
                  aes(x = direct_cert_pct, y =  rev_pp_local_state,
                      label = district),
                  size = text_repel_size) +
  geom_text_repel(data = current_modeling_data |>
                    filter(direct_cert_pct > .65),
                  aes(x = direct_cert_pct, y =  rev_pp_local_state,
                      label = district),
                  size = text_repel_size) +
geom_abline(color = "black") +
  ...
  labs(x = "Direct Certification %, FY23",
       ...
       size = "Enrollment",
       # shape = "LEA Type",
       title = "FIGURE 1: MS MAEP + Actual Local Per-Pupil Funding by District, FY24",
...
# Scatterplot of MAEP State Share PP --------

ggplot() +
  geom_point(data = current_modeling_data,
             aes(x = direct_cert_pct, y = maep_fy24_pp,
                 size = total_enroll, color = urbanicity),
             alpha = .8) +
  geom_text_repel(data = current_modeling_data |>
                    filter(maep_fy24_pp > 7500),
                  aes(x = direct_cert_pct, y =  maep_fy24_pp,
                      label = district),
                  size = text_repel_size) +
  geom_text_repel(data = current_modeling_data |>
  ...
  scale_size_area(max_size = 10,
                  labels = comma_format()) +
  labs(x = "Direct Certification %, FY23",
       y = "MAEP (State Share) Funding Per-Pupil, FY24",
       color = "Urbanicity",
       size = "Enrollment",
       # shape = "LEA Type",
       title = "FIGURE 2: MS MAEP Per-Pupil Funding by District, FY24",
       caption = "Sources: MS Department of Revenue, MS Department of Education, and U.S. Census Bureau") +
  theme_bw() +
  theme(text = element_text(family = "Avenir", size = 11),
        plot.caption = element_text(hjust = 0))

ggsave("figures/maep_state_share_pp_v_direct_cert.png", units = "in",
       height = 5.9, width = 10)


# Scatterplot of Actual local and expected local diff PP --------

ggplot() +
  geom_point(data = current_modeling_data,
             aes(x = direct_cert_pct, y = local_diff_pp,
                 size = total_enroll, color = urbanicity),
             alpha = .8) +
  geom_text_repel(data = current_modeling_data |>
                    filter(local_diff_pp > 5000),
                  aes(x = direct_cert_pct, y =  local_diff_pp,
                      label = district),
                  size = text_repel_size) +
  geom_text_repel(data = current_modeling_data |>
  ...
  labs(x = "Direct Certification % (Proxy measure for student poverty), FY23",
       y = "Per-Pupil Difference in actual FY22 local revenue & local share expectation, FY24",
       color = "Urbanicity",
       size = "Enrollment",
       # shape = "LEA Type",
       title = "FIGURE 3: Per-Pupil Difference in FY22 Actual Local and FY24 Expected Local",
       caption = "Sources: MS Department of Revenue, MS Department of Education, and U.S. Census Bureau") +
  theme_bw() +
  theme(text = element_text(family = "Avenir", size = 11),
        plot.caption = element_text(hjust = 0))

ggsave("figures/local_share_diff_pp_v_direct_cert.png", units = "in",
       height = 5.9, width = 10)


# Write csv -------

write_csv(current_modeling_data, "data/processed/current_modeling_addon_data.csv")
