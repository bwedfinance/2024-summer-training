...
ms_data <- read_csv("data/processed/ms_model7_matt_updates_addon_data.csv")
...
ms_model7 <- ms_data |>
  ...
  mutate(sparsity_weight_adj = case_when(st_per_sq_mile > sparsity_limit ~ 0,
         ...
         # total Model 7 funding 
         model_7_total_funding = base_funding + total_sped_funding + ell_funding + 
           ed_funding + gifted_funding + vocational_funding + sparsity_funding + conc_pov_funding,
         
         model_7_pp_funding = model_7_total_funding / total_enroll, 
         
         # Model 7 local share with the mills 
         model_7_local_mills_total = mill_1_value * max_mills,
         
         # Model 7 local share with 28% 
         model_7_local_pct_total = model_7_total_funding * max_local_pct, 
         
         # Apply the if_else to calculate the local share 
         model_7_local_share_total = ifelse(model_7_local_mills_total < model_7_local_pct_total, 
                                            model_7_local_mills_total, model_7_local_pct_total),
         model_7_local_share_pp = model_7_local_share_total / total_enroll,
         
         # Model 7 State share
         model_7_state_share = model_7_total_funding  - model_7_local_share_total,
         model_7_state_share_pp = model_7_state_share / total_enroll) |> 
  
  # Model 7 and current funding differences 
  mutate(model_7_diff_total = model_7_total_funding - maep_expected_local_total, 
         model_7_diff_pp = model_7_pp_funding - maep_expected_local_pp, 
         
         model_7_local_diff_total = model_7_local_share_total - local_revenue, 
         current_local_pp = local_revenue / total_enroll, 
         model_7_local_diff_pp = model_7_local_share_pp - current_local_pp,
         
         model_7_state_diff_pp = model_7_state_share_pp - maep_fy24_pp) |>
  ...
  mutate(sped_pp_funding = sped_funding / (sped_tier_1_enroll + sped_tier_2_enroll + sped_tier_3_enroll),
         sped_model_7_pp = total_sped_funding / (sped_tier_1_enroll + sped_tier_2_enroll + sped_tier_3_enroll),
         sped_pp_diff = sped_model_7_pp - sped_pp_funding) |>
  # Difference in per-pupil at risk funding 
  mutate(at_risk_pp_funding = at_risk_funding / econ_dis_enroll,
         ed_model_7_pp = ed_funding / econ_dis_enroll,
         ed_pp_diff = ed_model_7_pp - at_risk_pp_funding) |>
  # Difference in per-pupil vocational funding  
  mutate(cte_pp_funding = cte_funding / vocational_enroll,
         cte_model_7_pp = vocational_funding / vocational_enroll,
         cte_pp_diff = cte_model_7_pp - cte_pp_funding) |>
  
  
  # Take out Forest and charter schools 
  filter(mill_1_value_pp < 1000) |>
  
  select(dist_id, district, model_7_total_funding, model_7_diff_pp, direct_cert_pct, model_7_state_diff_pp, model_7_local_share_total, 
         model_7_diff_total, base_funding, sped_tier_1_funding, sped_tier_2_funding, sped_tier_3_funding, 
...
ms_model7_charters <- ms_model7 |>
  filter(dist_id == "2545" | dist_id == "1425" | dist_id == "4225" | 
           dist_id == "2525" | dist_id == "2515" | dist_id == "2535" | dist_id == "2505") |> 
  select(dist_id, district, model_7_pp_funding, maep_expected_local_pp, model_7_diff_pp, vocational_enroll)
...
ggplot() +
  geom_point(data = ms_model7,
             aes(x = maep_expected_local_pp, y = model_7_pp_funding,
                 size = total_enroll, color = direct_cert_pct),
             alpha = .8) +
  geom_text_repel(data = ms_model7 |>
                    filter(model_7_pp_funding > 9500),
                  aes(x = maep_expected_local_pp, y =  model_7_pp_funding,
                      label = district),
                  size = text_repel_size) +
  geom_text_repel(data = ms_model7 |>
                    filter(maep_expected_local_pp > 9000,
                           model_7_pp_funding < 9500),
                  aes(x = maep_expected_local_pp, y =  model_7_pp_funding,
                      label = district),
                  size = text_repel_size) +
  geom_text_repel(data = ms_model7 |>
  ...
  labs(x = "Total (MAEP + Expected Local) Funding Per-Pupil, FY24",
       y = "Model 7 Funding Per-Pupil, FY24",
       color = "Direct Cert %",
       size = "Enrollment",
       # shape = "LEA Type",
       title = "Change in FY24 Per-Pupil Funding, Model 7") +
  theme_bw() +
  theme(text = element_text(family = "Avenir", size = 11),
        plot.caption = element_text(hjust = 0))

ggsave("figures/updated_ms_model7/model7_vs_maep_local_expected.png", units = "in",
       height = 5.9, width = 10)

# SCATTERPLOT #2: Model 3 AND DIRECT CERT % -------
ggplot(ms_model7, aes(x = direct_cert_pct, y = model_7_pp_funding, size = total_enroll, color = urbanicity)) +
  geom_point(alpha = .8) +
  ...
  labs(x = "Direct Certification Percent, FY23",
       y = "Model 7 Per-Pupil Funding, FY24",
...
ggsave("figures/updated_ms_model7/model7_pp_direct_cert_pct.png", units = "in",
       height = 5.9, width = 10)

# SCATTERPLOT #3: MODEL 3 + VALUE OF 1 MILL ----
ggplot(ms_model7, aes(x = mill_1_value_pp, y = model_7_pp_funding, size = total_enroll, color = direct_cert_pct)) +
  geom_point(alpha = .8) +
  scale_color_viridis(labels = percent_format()) +
...
ggsave("figures/updated_ms_model7/model7_model_1_pp_value_1_mill.png", units = "in",
       height = 5.9, width = 10)

# Mapping --------

# merge model with map data
ms_model7_dist_map <- ms_shp |>
  left_join(ms_model7, by ="geoid") |>
  ...
  select(dist_id, model_7_diff_pp, everything())


# Per Pupil funding diff map
ggplot(ms_model7_dist_map) +
  geom_sf(aes(fill = model_7_diff_pp),
          color = "grey54",
          size = .5) +
...
ggsave("figures/updated_ms_model7/pp_diff_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/updated_ms_model7/ell_pp_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/updated_ms_model7/sparsity_pp_map.png", units = "in",
       height = 5.9, width = 6)
...
#         plot.caption = element_text(hjust = 0))
# 
# ggsave("figures/ms_model7/conc_pov_pp_map.png", units = "in",
#        height = 5.9, width = 6)
...
ggsave("figures/updated_ms_model7/sped_pp__diff_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/updated_ms_model7/ed_pp_diff_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/updated_ms_model7/cte_pp_diff_map.png", units = "in",
       height = 5.9, width = 6)


# state summary --------
model7_state_summary <- ms_model7 |>
  summarise(model_7_total_funding = sum(model_7_total_funding, na.rm = T),
            ...
            model_7_state_share = sum(model_7_state_share, na.rm = T),
  ...
  mutate(maep_total_cost = maep_rev_fy24 + local_share_expected_total,
         total_difference = model_7_total_funding - maep_total_cost,
         state_share_diff = model_7_state_share - maep_rev_fy24)

# Modeling data for the MS coalition -------
ms_model7_working_session <- ms_model7 |>
  select(-geoid, -model_7_local_diff_pp, - shortName,
         ...
         -local_revenue_total, -local_revenue_pp, - mpv, -mhi, -model_7_local_pct_total, 
         -current_local_pp, -ed_model_7_pp, -local_revenue, -federal_revenue,
         -local_share_pp_pct, -urbanicity, -at_risk_pp_funding, -model_7_local_diff_total,
         -sped_pp_funding, -sped_model_7_pp, - model_7_state_diff_pp, -maep_rev_fy23,
         -maep_fy23_pp, -cte_model_7_pp, -cte_pp_diff) |>
  select(dist_id, district, total_enroll, model_7_total_funding, model_7_pp_funding, 
         model_7_diff_total, model_7_diff_pp, model_7_state_share,  model_7_state_share_pp, 
         model_7_local_share_total, model_7_local_share_pp, model_7_local_mills_total,
...
write_csv(ms_model7, "data/processed/ms_model7_matt_updates_addon_data.csv")

write_csv(ms_model7_working_session, "data/processed/ms_model7_2023-12-14_data.csv")
