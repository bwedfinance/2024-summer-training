...
ms_model4 <- ms_data |>
  ...
  select(dist_id, district, model_4_total_funding, model_4_pp_funding, model_4_diff_pp, model_4_local_share_total, 
         model_4_diff_total, base_funding, sped_tier_1_funding, sped_tier_2_funding, sped_tier_3_funding, 
...
ms_model4_charters <- ms_model4 |>
  filter(dist_id == "2545" | dist_id == "1425" | dist_id == "4225" | 
           dist_id == "2525" | dist_id == "2515" | dist_id == "2535" | dist_id == "2505") |> 
  select(dist_id, district, model_4_pp_funding, maep_expected_local_pp, model_4_diff_pp)
...
ggplot() +
  ...
  geom_text_repel(data = ms_model4 |>
                    filter(maep_expected_local_pp < 7300),
                  aes(x = maep_expected_local_pp, y =  model_4_pp_funding,
                      label = district),
  ...
  geom_text_repel(data = ms_model4 |>
                  ...
                  size = text_repel_size) +
  geom_text_repel(data = ms_model4 |>
                    filter(model_4_pp_funding > 9000,
                           maep_expected_local_pp < 9000),
                  aes(x = maep_expected_local_pp, y =  model_4_pp_funding,
                      label = district),
                  size = text_repel_size) +
  geom_text_repel(data = ms_model4 |>
                    filter(maep_expected_local_pp < 8600 &
                             maep_expected_local_pp > 8300 & 
                             model_4_pp_funding < 8250),
                  aes(x = maep_expected_local_pp, y = model_4_pp_funding,
                      label = district),
                  size = text_repel_size) +
...
ggsave("figures/updated_ms_model4/model4_vs_maep_local_expected.png", units = "in",
       height = 5.9, width = 10)
...
ggsave("figures/updated_ms_model4/model4_pp_direct_cert_pct.png", units = "in",
       height = 5.9, width = 10)
...
ggsave("figures/updated_ms_model4/model4_model_1_pp_value_1_mill.png", units = "in",
       height = 5.9, width = 10)
...
ggsave("figures/updated_ms_model4/pp_diff_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/updated_ms_model4/ell_pp_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/updated_ms_model4/sparsity_pp_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/updated_ms_model4/sped_pp__diff_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/updated_ms_model4/ed_pp_diff_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/updated_ms_model4/cte_pp_diff_map.png", units = "in",
       height = 5.9, width = 6)
...
model4_state_summary <- ms_model4 |>
  ...
  mutate(maep_total_cost = maep_rev_fy24 + local_share_expected_total,
         total_difference = model_4_total_funding - maep_total_cost,
         state_share_diff = model_4_state_share - maep_rev_fy24)

# Modeling data for the MS coalition -------
ms_model6_working_session <- ms_model4 |>
  select(-geoid, -model_4_local_diff_pp, - shortName,
         ...
         -local_share_pp_pct, -urbanicity, -at_risk_pp_funding, -model_4_local_diff_total,
         -sped_pp_funding, -sped_model_4_pp, -maep_rev_fy23,
         -maep_fy23_pp, -cte_model_4_pp, -cte_pp_diff) |>
  select(dist_id, district, total_enroll, model_4_total_funding, model_4_pp_funding, 
         ...
         model_4_local_share_total, model_4_local_share_pp, model_4_local_mills_total,
...
write_csv(ms_model4, "data/processed/ms_model4_matt_updates_addon_data.csv")

write_csv(ms_model6_working_session, "data/processed/ms_model6_2023-12-14_data.csv")
