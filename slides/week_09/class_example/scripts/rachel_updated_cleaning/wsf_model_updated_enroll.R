...
ms_data <- read_csv("data/processed/updated_2024-04-16-ms_modeling_data.csv") |>
  select(dist_id, district, everything())
...
ms_model9 <- ms_data |>
  ...
  mutate(sparsity_weight_adj = case_when(students_sq_mile > sparsity_limit ~ 0,
                                         # if districts are between the max and min rates, the get a scaled
                                         # version of the concentrated poverty weight
                                         students_sq_mile <= sparsity_limit ~ (sparsity_limit - students_sq_mile) /
                                           100,
...
# Difference in funding with Matt's model 
  mutate(matt_total_diff = matt_wsf_total - maep_expected_local_total, 
         matt_state_share =  matt_wsf_total  - model_9_local_share_total, 
         matt_state_share_diff = matt_state_share - local_revenue) |>
  
  
  # Take out Forest and charter schools 
  filter(mill_1_value_pp < 1000) |>
  
  select(dist_id, district, matt_total_diff, matt_state_share_diff, everything()) 
...
ggsave("figures/2024-04-16_modeling/model9_vs_maep_local_expected.png", units = "in",
       height = 5.9, width = 10)

# SCATTERPLOT #2: Model 9 AND DIRECT CERT % -------
ggplot(ms_model9, aes(x = direct_cert_pct, y = matt_wsf_pp, size = total_enroll, color = urbanicity)) +
  geom_point(alpha = .8) +
  ...
  labs(x = "Direct Certification % (Proxy measure for student poverty), FY24",
       y = "INSPIRE Per-Pupil Funding",
       title = "INSPIRE Per-Pupil Funding, FY25",
       size = "Enrollment",
       color = "Urbanicity") +
  theme_bw() +
...
ggsave("figures/2024-04-16_modeling/model9_pp_direct_cert_pct.png", units = "in",
       height = 5.9, width = 10)
...
ggplot(ms_model9, aes(x = direct_cert_pct, y = maep_expected_local_pp, size = total_enroll, color = urbanicity)) +
  ...
  # scale_size_area(labels = comma, max_size = 10) +
  labs(x = "Direct Certification % (Proxy measure for student poverty), FY24",
       y = "Total Current (MAEP + Expected Local) Funding Per-Pupil",
       title = "MS Total Current (MAEP + Expected Local) Per-Pupil Funding, FY25",
       size = "Enrollment",
       color = "Urbanicity") +
  theme_bw() +
  theme(text = element_text(family = "Avenir", size = 12),
...
ggsave("figures/2024-04-16_modeling/model9_maep_pp_direct_cert_pct.png", units = "in",
       height = 5.9, width = 10)
...
ggsave("figures/2024-04-16_modeling/model9_model_1_pp_value_1_mill.png", units = "in",
       height = 5.9, width = 10)
...
ggsave("figures/2024-04-16_modeling/pp_diff_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/2024-04-16_modeling/ell_pp_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/2024-04-16_modeling/sparsity_pp_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/2024-04-16_modeling/sped_pp__diff_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/2024-04-16_modeling/ed_pp_diff_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/2024-04-16_modeling/cte_pp_diff_map.png", units = "in",
       height = 5.9, width = 6)
...
ggsave("figures/2024-04-16_modeling/state_share_pp_diff_map.png", units = "in",
       height = 5.9, width = 6)
